...
async function getIdByGuid(guid, callback) {
    console.log("teamsProvider->getIdByGuid called with guid " + guid);
    const db = new sqlite3.Database(sharedDBMethods.returnDBPath(), (error) => {
        if (error) {return console.log(error.message);}
        db.serialize(() => {
            let result = -1;
            db.get(`SELECT id FROM teams WHERE guid=? LIMIT 1`, [guid], (error, row) => {
                console.log("teamsProvider->getIdByGuid returned row obj " + JSON.stringify(row));
                console.log("teamsProvider->getIdByGuid sql Error:" + error);
                result = row.id;
                console.log("teamsProvider->getIdByGuid this is the result: " + result);
    ...
    const db = new sqlite3.Database(sharedDBMethods.returnDBPath(), (error) => {
        ...
        else
        {
            db.serialize(() => {
                ...
                console.log("teamsProvider->save Generated guid for new record: " + uniqueUUID);
